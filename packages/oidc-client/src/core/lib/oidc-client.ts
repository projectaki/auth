import { createDefaultLogger } from "./_logger";
import {
  Session,
  createAuthUrl,
  createDiscoveryUrl,
...
const createOidcClient = ({
  ...
  const refreshTokens = async (): Promise<void> => {
    ...
    const isValid = validateIdToken(
      newAuthResult.id_token,
      config,
      appState.nonce,
      appState.max_age,
    );
  ...
  const _setAuthState = (authState: AuthenticationState) => {
    ...
    if (typeof actions.authStateChange === "function")
      actions.authStateChange(authState);
  };

  const _loadDiscoveryDocument = async () => {
    try {
      if (!(await _loadDiscoveryDocumentFromStorage()))
        await _loadDiscoveryDocumentFromWellKnown();
  ...
  const _loadDiscoveryDocumentFromWellKnown =
  ...
  const _createAuthUrlAndSaveState = async (extraParams?: ExtraQueryParams) => {
    const state = await createNonce(32, config);
  ...
  const _createLogoutUrlAndSaveState = async (
  ...
  const authCallback = async () => {
    ...
    const isValidIdToken = validateIdToken(
      res.id_token,
      config,
      appState.nonce,
      appState.max_age,
    ...
    if (appState.sendUserBackTo)
      actions.replaceUrlState(appState.sendUserBackTo);
  };
  ...
  const _handleCodeFlowRedirect = async (
  ...
  const _fetchTokensWithRefreshToken = async (
    appState: SessionParams,
  ): Promise<Session> => {
    ...
    const tokenResponse = await httpService.post<Session>(
